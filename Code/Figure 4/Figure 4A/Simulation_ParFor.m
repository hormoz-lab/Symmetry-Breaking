% === Set Flag ===
check_existing = false;  % Set to false to skip filtering already-run combos
Sample         = 6;      % (6, 7, 8, 9, 10) Sample number to load
num_task       = 3000;   % Number of tasks per script
...
% Parent directory containing data
% parent_path               = '/n/scratch/users/s/suw469/gastruloids_1500_25%_check';
parent_path               = '/n/data1/hms/sysbio/hormoz/users/suxuan/gastruloids_1500_25%_check';
sample_folder             = fullfile(parent_path, ['OUTPUT', num2str(Sample)]);
...
input_file_path_G         = fullfile(sample_folder, 'WorkSpace_MatrixG_Initial.csv');

% Total number of cells
MatrixP                   = cell2mat(table2cell(readtable(input_file_path_P)));
num_cells                 = size(MatrixP, 1);

% Load position and gene expression noise
noise_filename            = fullfile(parent_path, 'Noise.mat');             % Path to noise file
load(noise_filename, 'Noise');                                              % Loads a 2×SAMPLE cell array, 2 include position and gene noise (though here we only use position)



%% --- Section 3: Prepare Scanning Library ---

% Load all alpha and betaL combinations from scanning library
scanning_library_filename = fullfile(parent_path, 'scanning_library.mat');  % Path to scanning library file
...
if check_existing
    % Find Already Existing Output Files
    csv_files = dir(fullfile(sample_folder, '*_MatrixP_Final.csv'));
    existing_combinations = [];

    % Pattern to match filenames with all 7 parameter values
    pattern = ['Aii_([\d\.\-eE]+)_' ...
               'Aoo_([\d\.\-eE]+)_' ...
               'Aio_([\d\.\-eE]+)_' ...
               'BLii_([\d\.\-eE]+)_'...
    ...
    % Extract parameter values from filenames
    for i = 1:length(csv_files)
        fname = csv_files(i).name;
        tokens = regexp(fname, pattern, 'tokens');
        if ~isempty(tokens)
            vals = str2double(tokens{1});
            existing_combinations = [existing_combinations; vals];
        end
    end

    % Filter out combinations that have already been run
    if isempty(existing_combinations)
        combinations_to_run = scanning_library;
    else
        rounded_existing = round(existing_combinations, 4);
        rounded_library  = round(scanning_library, 4);
        [~, ia] = setdiff(rounded_library, rounded_existing, 'rows');
        combinations_to_run = scanning_library(ia, :);
    end

    fprintf("✅ Found %d unused combinations (out of %d total).\n", ...
            size(combinations_to_run, 1), size(scanning_library, 1));
else
    % ⏩ Skip filtering and run all combinations
    combinations_to_run = scanning_library;
    fprintf("⚠️ Skipping existing file check: using all %d combinations.\n", ...
            size(combinations_to_run, 1));
end

num_combos = size(combinations_to_run, 1);


%% --- Section 4: Parallel Simulation Execution ---

% Auto-derive loop range from file name 
[~, script_name, ~] = fileparts(mfilename('fullpath'));       % Get the name of the current script
tokens = regexp(script_name, '_(\d+)$', 'tokens');            % Extract the final number from the script name (assumes it always ends with "_<number>")
if isempty(tokens)
    error('Could not extract the script number from file name: %s', script_name);
end
chunk_number = str2double(tokens{1}{1});

start_idx = (chunk_number - 1) * num_task + 1;                % Calculate start and end indices for this chunk
end_idx   = chunk_number * num_task;

if start_idx > num_combos                                     % No work left — return will stop the entire script
    fprintf('⚠️ Chunk %d has no combinations to run. Skipping.\n', chunk_number);
    return
elseif end_idx > num_combos                                   % Trim end_idx to the last combo
    end_idx = num_combos;
end

fprintf('▶ Running chunk %d: combo_idx %d to %d\n', chunk_number, start_idx, end_idx);
...
parfor combo_idx = start_idx:end_idx

    fprintf('Running combo_idx = %d\n', combo_idx); 
    ...
    alpha_ii = combinations_to_run(combo_idx, 1); % ii: Short-range force between inner cell and inner cell
    alpha_oo = combinations_to_run(combo_idx, 2); % oo: Short-range force between outer cell and outer cell
    alpha_io = combinations_to_run(combo_idx, 3); % io: Short-range force between inner cell and outer cell (io, oi order does not matter right now)
    ...
    betaL_ii = combinations_to_run(combo_idx, 4); % ii: Long-range force from inner cell to inner cell
    betaL_oo = combinations_to_run(combo_idx, 5); % oo: Long-range force from outer cell to outer cell
    betaL_io = combinations_to_run(combo_idx, 6); % io: Long-range force from inner cell to outer cell (order matter !)
    betaL_oi = combinations_to_run(combo_idx, 7); % oi: Long-range force from outer cell to inner cell (order matter !)
    ...
    for t = Tmax*2 + dt : dt : Tmax*5
        ...
        noise_term               = kappaF * sqrt(dt) * Noise{1, Sample}(:, :, round(t / dt_ini));
    ...
    file_base = sprintf( ...
        'Aii_%.3f_Aoo_%.3f_Aio_%.3f_BLii_%.3f_BLoo_%.3f_BLio_%.3f_BLoi_%.3f_D1_%d_dt_%.2f', ...
        alpha_ii, alpha_oo, alpha_io, betaL_ii, betaL_oo, betaL_io, betaL_oi, D1, dt);

    output_file_path_P = fullfile(sample_folder, [file_base '_MatrixP_Final.csv']);
    output_file_path_G = fullfile(sample_folder, [file_base '_MatrixG_Final.csv']);
