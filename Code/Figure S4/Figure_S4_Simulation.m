...
load(noise_filename, 'Noise');    % Noise{1,Sample} = position noise

% Root output directory
output_path = '/home/suw469/codes/Gastruloid_Modeling/Modeling/Check/Visualization/Noise_Test';

% Define test sets
kappa_sets = {
    'Test_1', [0.001, 0.01, 0.1, 1, 10]; ...
    'Test_2', [0.2, 0.15, 0.1, 0.05, 0] ...
};
...
%% --- Section 3: Loop over test sets and kappa values ---
for set_idx = 1:size(kappa_sets,1)

    test_name   = kappa_sets{set_idx,1};
    kappa_list  = kappa_sets{set_idx,2};

    % Make test subfolder
    output_root = fullfile(output_path, test_name);
    if ~exist(output_root, 'dir'), mkdir(output_root); end
    for s = 6:10
        out_dir_s = fullfile(output_root, sprintf('OUTPUT%d', s));
        if ~exist(out_dir_s, 'dir'), mkdir(out_dir_s); end
    end

    % Loop over kappa values
    for kappaF = kappa_list
        
        fprintf('>>> Running %s with kappaF = %.3f\n', test_name, kappaF);

        % --- Section 4: Run Simulation for 5 Samples ---
        parfor Sample = 6:10
        
            sample_folder             = fullfile(parent_path, ['OUTPUT', num2str(Sample)]);
            sample_dir = fullfile(output_root, sprintf('OUTPUT%d', Sample));
            ...
            for t = Tmax*2 + dt : dt : Tmax*5
                ...
                % frame_idx = round(t / dt);                 % integer index per your code
            ...
            file_base = sprintf( ...
                'Aii_%.3f_Aoo_%.3f_Aio_%.3f_BLii_%.3f_BLoo_%.3f_BLio_%.3f_BLoi_%.3f_D1_%d_dt_%.2f_kappaF_%.3f', ...
                alpha_ii, alpha_oo, alpha_io, betaL_ii, betaL_oo, betaL_io, betaL_oi, D1, dt, kappaF);
        
            output_file_path_P = fullfile(sample_dir, [file_base '_MatrixP_Final.csv']);
            output_file_path_G = fullfile(sample_dir, [file_base '_MatrixG_Final.csv']);
