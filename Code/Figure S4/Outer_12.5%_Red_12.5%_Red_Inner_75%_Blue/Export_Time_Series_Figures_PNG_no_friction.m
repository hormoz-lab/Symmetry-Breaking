...
parent_path    = '/n/data1/hms/sysbio/hormoz/users/suxuan/Materials/Codes/Virtual_Experiment/Experiment 3/Outer_12.5%_Red_12.5%_Red_Inner_75%_Blue/Simulation_Time_500_3000_no_friction';
...
for n = OUTPUTS
    ...
    for t = T_list
        ...
        % Step 2.2.3: Compute the center and half-span of the largest cluster
        [center, half_span] = compute_center_and_half_span(MatrixP);

        % Step 2.2.4: Record per-frame selection into the collection table    
        collection_table.Filename{k} = filename;
        collection_table.Position{k} = MatrixP;
        collection_table.Gene2{k}    = MatrixG(:, 2);
        collection_table.Center{k}   = center;

        % Step 2.2.5: Update the global half-span (used for a fixed visualization scale)
