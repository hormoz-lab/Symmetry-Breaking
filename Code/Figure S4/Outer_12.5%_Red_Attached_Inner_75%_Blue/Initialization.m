...
output_path      = '/n/data1/hms/sysbio/hormoz/users/suxuan/Materials/Codes/Virtual_Experiment/Experiment 3/Outer_12.5%_Red_Attached_Inner_75%_Blue/Initial_Status';
...
%% Step 2: Remove 12.5% outer cells at periphery from MatrixP and MatrixG 
for n = OUTPUTS
    ...
    % Step 2.3: Identify the farthest 50% of red (outer) cells and remove them
    ...
    keep(top_red_indices) = false;                                         % Mark the outermost (red) cells as false (to be removed)
    MatrixP_pruned = MatrixP_initial(keep, :);
    MatrixG_pruned = MatrixG_initial(keep, :);
    
    total_cells   = size(MatrixP_initial, 1);
    removed_cells = numel(top_red_indices); 
