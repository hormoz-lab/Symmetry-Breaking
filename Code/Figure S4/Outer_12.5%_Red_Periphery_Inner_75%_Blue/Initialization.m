...
output_path      = '/n/data1/hms/sysbio/hormoz/users/suxuan/Materials/Codes/Virtual_Experiment/Experiment 3/Outer_12.5%_Red_Periphery_Inner_75%_Blue/Initial_Status';
...
%% Step 2: Remove 12.5% outer cells attached from MatrixP and MatrixG 
for n = OUTPUTS
    ...
    % Step 2.4: Identify the nearest 50% of red (outer) cells and remove them
    outer_indices = find(MatrixG_initial(:, 2) == 1);                      % Row indices of all outer cells
    bottom_red_indices = setdiff(outer_indices, top_red_indices);          % Remaining (remove)
    keep = true(size(MatrixP_initial, 1), 1);                              % Create a logical mask that initially keeps all cells
    keep(bottom_red_indices) = false;                                      % Mark the nearest outer cells as false (to be removed)
    MatrixP_pruned = MatrixP_expanded(keep, :);
    MatrixG_pruned = MatrixG_initial(keep, :);
    
    total_cells   = size(MatrixP_initial, 1);
    removed_cells = numel(bottom_red_indices); 
    remaining     = size(MatrixP_pruned, 1);

    fprintf('Original total cells: %d\n', total_cells);
    fprintf('Removed outer cells:  %d\n', removed_cells);
    fprintf('Remaining cells:      %d\n', remaining);
        
    % Step 2.5 (optional): Visualize before and after expansion (uses col 2 as gene_2) for comparison 
    % Before expansion
    ...
    % After expansion
    draw_cell_structure(MatrixP_pruned, MatrixG_pruned(:, 2), [0, 0, 0], ...
                        global_half_span, cell_radius, num_face, ...
                        fig_visibility, FIG_HEIGHT, FIG_WIDTH);

    % Step 2.5: Save outputs
    output_file_P   = fullfile(output_folder, 'WorkSpace_MatrixP_Pruned.csv');
    writematrix(MatrixP_pruned, output_file_P);
    output_file_G   = fullfile(output_folder, 'WorkSpace_MatrixG_Pruned.csv');
    writematrix(MatrixG_pruned, output_file_G);
end
