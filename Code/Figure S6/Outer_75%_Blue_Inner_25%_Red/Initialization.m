...
output_path      = '/n/data1/hms/sysbio/hormoz/users/suxuan/Materials/Codes/Virtual_Experiment/Experiment 4/Outer_75%_Blue_Inner_25%_Red/Initial_Status';
if ~exist(output_path, 'dir')
    mkdir(output_path);
end

% Reverse MatrixG parameter sets
GeneNum = 3;
percentage = 0.25;
...
%% Step 2: Copy MatrixP and define reversed MatrixG to swap inner and outer cell layers
for n = OUTPUTS
    ...
    % Step 2.3: Define reversed MatrixG where the inner 25% of cells have Gene 2 = 1
    population = size(MatrixP_initial, 1);                                 % Total number of cells
    MatrixG_reversed = NaN * ones(population, GeneNum);                    % Initialize with NaNs
    MatrixG_reversed(:) = 0;                                               % Set all gene values to zero

    distance_to_origin = sqrt(sum(MatrixP_initial.^2, 2));                 % Compute distance of each cell from origin
    [~, sorted_indices] = sort(distance_to_origin, 'ascend');              % Sort distances in ascending order
    top_n = round(percentage * population);
    top_indices = sorted_indices(1:top_n);
    MatrixG_reversed(top_indices, 2) = 1;                                  % Set Gene 2 = 1 for those nearest cells

    % Step 2.4 (optional): Visualize before and after reverse MatrixG (uses col 2 as gene_2) for comparison 
    % Before reverse
    ...
    % After reverse
    draw_cell_structure(MatrixP_initial, MatrixG_reversed(:, 2), [0, 0, 0], ...
                        global_half_span, cell_radius, num_face, ...
                        fig_visibility, FIG_HEIGHT, FIG_WIDTH);
    ...
    output_file_G   = fullfile(output_folder, 'WorkSpace_MatrixG_Reversed.csv');
    writematrix(MatrixG_reversed, output_file_G);
end
